...
{
  report_debug_message("received gcode");
  /* -------------------------------------------------------------------------------------
  ...
  */  
  ...
  if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
    ...
    if (axis_command == AXIS_COMMAND_MOTION_MODE) { 
	 ...
	 if (gc_block.modal.units == UNITS_MODE_STEP) {FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND);} //KEYME TODO ERROR
    ...
    if (gc_state.modal.feed_rate == FEED_RATE_MODE_UNITS_PER_MIN) { // Last state is also G94
      if (bit_istrue(value_words,bit(WORD_F))) {
        ...
        else if (gc_block.modal.units == UNITS_MODE_STEP) {gc_block.values.f = single_step_speed; } //TODO: not sure about this yet.
      } else {
    ...
    // the value must be positive. In inverse time mode, a positive value must be passed with each block.
      ...
      switch (gc_block.modal.motion) {
        ...
        case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
          ...
          else if (gc_block.modal.units == UNITS_MODE_STEP) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION);} //can't do arc by steps. TODO: what's the best errror code? 
